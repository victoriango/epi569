...
sd_index_incubation_days <- sd(data$INDEX_INCUBATION_DAYS, na.rm=TRUE) #creating the sd of incubation period to calculate dispersion
...
-   Because incubation periods are log-normally distributed, 66% of cases should fall within the median/exp(sd) and median\*exp(sd). Does that appear to be the case?
    ...
    ```{r}
    median_incu<-median(data$INDEX_INCUBATION_DAYS, na.rm=TRUE)
    dispersion_incu<-exp(sd_index_incubation_days)
    median_incu/dispersion_incu #division
    median_incu*dispersion_incu #multiplication
