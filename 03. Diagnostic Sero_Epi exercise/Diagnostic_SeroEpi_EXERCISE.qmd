...
mean_pcr_ct <- sero %>%
  filter(Age_Month == 36) %>%
  group_by(EIA) %>%
  summarize(mean_PCR_Ct = mean(PCR_Ct, na.rm = TRUE))
...
The IgG/IgA reflects the immune system's long-term response, whereas the EIA/PCR reflect both current infection and historical responses due to testing viral genetic material and antibody response.
